...
const JobPostingsList = () => {
  ...
  const filteredApplications = allCandidates?.filter((candidate) => {
    return candidate?.applicantName?.toLowerCase()?.includes(searchQuery?.toLowerCase());
  });
    ...
    <VStack textAlign={"left"} spacing={4} py={4} className="setPX" px={6}>
      ...
      {!searchQuery ? (
        <Box w={'100%'}>
          ...
          >
            <Box overflow={"hidden"} className="w-50" spacing={4}>
              <Box
                overflowX={"scroll"}
                display={"flex"}
                minWidth={"max-content"}
        ...
        <Table variant="simple" colorScheme="red">
                  ...
                  <Tbody>
                    {filteredApplications && filteredApplications?.map((candidate) =>
                      (
                        <Tr key={Math.random()}>
                          <Td>{candidate.applicantName} </Td>
                          <Td>{candidate.applicantEmail}</Td>
